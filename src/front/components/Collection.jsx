...
const Collection = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const respUser = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/user-collection`, {
                    headers: {
                ...
                // Fetch deck cards
                const respDeck = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/user-deck`, {
                    headers: {
                        "Authorization": `Bearer ${accessToken}`,
                        "Content-Type": "application/json"
                    }
                ...
                if (userData.collection) {
                    userData.collection.forEach(card => {
                        userCardsMap[card.id] = card.quantity;
                    });
                }
                setAllCards(all);
                setUserCollection(userCardsMap);
                setDeckCards(deckData.deck || []);
            } catch (err) {
                setAllCards([]);
                setUserCollection({});
                setDeckCards([]);
    ...
    const handleAddToDeck = async (cardId) => {
        const accessToken = localStorage.getItem("accessToken");
        try {
            const resp = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/user-deck`, {
                method: "POST",
                headers: {
                    "Authorization": `Bearer ${accessToken}`,
                    "Content-Type": "application/json"
            ...
            if (!resp.ok) {
                alert(data.msg || "Error adding card to deck");
            } else {
